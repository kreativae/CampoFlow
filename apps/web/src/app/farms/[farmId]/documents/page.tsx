...
export default function DocumentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<FarmDocument[]>(`/farms/${farmId}/documents`, {
        token: accessToken,
      });
      setDocuments(data);
    } catch (err) {
  ...
  async function handleUpload(event: FormEvent) {
    ...
    try {
      ...
      await apiUpload(`/farms/${farmId}/documents`, formData, accessToken);
  ...
  async function handleDownload(doc: FarmDocument) {
    setError(null);
    try {
      await apiDownload(`/farms/${farmId}/documents/${doc.id}/download`, doc.fileName, accessToken);
    } catch (err) {
  ...
  async function handleDelete(documentId: string) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/documents/${documentId}`, {
        method: 'DELETE',
        token: accessToken,
      });
      await loadData();
