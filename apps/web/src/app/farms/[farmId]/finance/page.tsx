...
export default function FinancePage() {
  ...
  const loadTransactions = useCallback(async () => {
    try {
      const data = await apiFetch<Transaction[]>(`/farms/${farmId}/transactions`, {
        token: accessToken,
      });
  ...
  const loadCashFlow = useCallback(async () => {
    try {
      const data = await apiFetch<CashFlowBucket[]>(
        `/farms/${farmId}/finance/cash-flow?granularity=${granularity}`,
        { token: accessToken },
      );
      setCashFlow(data);
    } catch (err) {
  ...
  async function handleCreate(event: FormEvent) {
    ...
    try {
      ...
      await apiFetch<Transaction>(`/farms/${farmId}/transactions`, {
        method: 'POST',
        token: accessToken,
        body: {
          type,
  ...
  async function handleMarkPaid(transactionId: string) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/transactions/${transactionId}/pay`, {
        method: 'PATCH',
        token: accessToken,
      });
      await loadData();
  ...
  async function handleDelete(transactionId: string) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/transactions/${transactionId}`, {
        method: 'DELETE',
        token: accessToken,
      });
      await loadData();
