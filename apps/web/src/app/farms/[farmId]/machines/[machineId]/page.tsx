...
export default function MachineDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<Machine>(`/farms/${farmId}/machines/${machineId}`, {
        token: accessToken,
      });
      setMachine(data);
    } catch (err) {
  ...
  async function handleAddMaintenance(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/machines/${machineId}/maintenances`, {
        method: 'POST',
        token: accessToken,
        body: {
  ...
  async function handleAddFuel(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/machines/${machineId}/fuel-records`, {
        method: 'POST',
        token: accessToken,
        body: {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
      <header className="mb-8">
        <Link href={`/farms/${farmId}/machines`} className="text-sm text-green-700 hover:underline">
          ← Máquinas
        </Link>
