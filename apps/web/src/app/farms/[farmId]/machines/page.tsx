...
export default function MachinesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [machinesData, costsData] = await Promise.all([
        apiFetch<Machine[]>(`/farms/${farmId}/machines`, { token: accessToken }),
        apiFetch<MachineCostSummary[]>(`/farms/${farmId}/machines/costs`, { token: accessToken }),
      ]);
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch<Machine>(`/farms/${farmId}/machines`, {
        method: 'POST',
        token: accessToken,
        ...
        <ul className="space-y-2">
          {machines.map((machine) => {
            ...
            return (
              <li key={machine.id}>
                <Link
                  href={`/farms/${farmId}/machines/${machine.id}`}
