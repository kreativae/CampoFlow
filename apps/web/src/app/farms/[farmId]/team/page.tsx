...
export default function TeamPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [tasksData, workLogsData, shiftsData] = await Promise.all([
        apiFetch<Task[]>(`/farms/${farmId}/tasks`, { token: accessToken }),
        apiFetch<WorkLog[]>(`/farms/${farmId}/work-logs`, { token: accessToken }),
        apiFetch<Shift[]>(`/farms/${farmId}/shifts`, { token: accessToken }),
      ]);
      setTasks(tasksData);
      ...
      try {
        const membersData = await apiFetch<Member[]>(`/farms/${farmId}/members`, {
          token: accessToken,
        });
  ...
  async function handleCreateTask(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/tasks`, {
        method: 'POST',
        token: accessToken,
  ...
  async function handleUpdateStatus(taskId: string, status: TaskStatus) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/tasks/${taskId}`, {
        method: 'PATCH',
        token: accessToken,
        body: { status },
      });
  ...
  async function handleAddWorkLog(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/work-logs`, {
        method: 'POST',
        token: accessToken,
  ...
  async function handleCreateShift(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/shifts`, {
        method: 'POST',
        token: accessToken,
