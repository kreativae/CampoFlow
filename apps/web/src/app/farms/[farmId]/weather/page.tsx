...
export default function WeatherPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const base = `/farms/${farmId}/weather`;
      const [latestData, alertsData, historyData] = await Promise.all([
        apiFetch<WeatherRecord | null>(`${base}/latest`, { token: accessToken }),
        apiFetch<WeatherRecord[]>(`${base}/alerts`, { token: accessToken }),
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/weather`, {
        method: 'POST',
        token: accessToken,
        body: {
