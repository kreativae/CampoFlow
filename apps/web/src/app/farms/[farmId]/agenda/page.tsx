...
export default function AgendaPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [eventsData, alertsData] = await Promise.all([
        apiFetch<AgendaEvent[]>(`/farms/${farmId}/agenda`, { token: accessToken }),
        apiFetch<AgendaAlert[]>(`/farms/${farmId}/agenda/alerts`, { token: accessToken }),
      ]);
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch<AgendaEvent>(`/farms/${farmId}/agenda`, {
        method: 'POST',
        token: accessToken,
  ...
  async function handleComplete(eventId: string) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/agenda/${eventId}/complete`, {
        method: 'PATCH',
        token: accessToken,
      });
      await loadData();
