...
export default function NotificationsPage() {
    ...
    async (unreadOnly: boolean) => {
      ...
      try {
        const data = await apiFetch<AppNotification[]>(
          `/farms/${farmId}/notifications${unreadOnly ? '?unreadOnly=true' : ''}`,
          { token: accessToken },
        );
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/notifications/generate`, {
        method: 'POST',
        token: accessToken,
      });
  ...
  async function handleMarkRead(id: string) {
    try {
      await apiFetch(`/farms/${farmId}/notifications/${id}/read`, {
        method: 'PATCH',
        token: accessToken,
      });
      await loadData(showUnreadOnly);
  ...
  async function handleMarkAllRead() {
    try {
      await apiFetch(`/farms/${farmId}/notifications/read-all`, {
        method: 'PATCH',
        token: accessToken,
      });
      await loadData(showUnreadOnly);
  ...
  return (
    <div className="mx-auto max-w-3xl p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link href={`/farms/${farmId}`} className="text-sm text-blue-600 hover:underline">
