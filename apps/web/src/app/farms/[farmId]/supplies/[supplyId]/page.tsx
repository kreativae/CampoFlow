...
export default function SupplyDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<Supply>(`/farms/${farmId}/supplies/${supplyId}`, {
        token: accessToken,
      });
      setSupply(data);
    } catch (err) {
  ...
  async function handleAddMovement(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/supplies/${supplyId}/movements`, {
        method: 'POST',
        token: accessToken,
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
      <header className="mb-8">
        <Link href={`/farms/${farmId}/supplies`} className="text-sm text-green-700 hover:underline">
          ← Insumos
        </Link>
