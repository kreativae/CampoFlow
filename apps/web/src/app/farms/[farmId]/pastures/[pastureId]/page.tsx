...
export default function PastureDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<Pasture>(`/farms/${farmId}/pastures/${pastureId}`, {
        token: accessToken,
      });
      setPasture(data);
    } catch (err) {
  ...
  async function handleEnter(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/pastures/${pastureId}/occupations`, {
        method: 'POST',
        token: accessToken,
  ...
  async function handleExit(occupationId: string) {
    setError(null);
    try {
      await apiFetch(`/farms/${farmId}/pastures/${pastureId}/occupations/${occupationId}/exit`, {
        method: 'PATCH',
        token: accessToken,
      });
      await loadData();
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
      <header className="mb-8">
        <Link href={`/farms/${farmId}/pastures`} className="text-sm text-green-700 hover:underline">
          ← Pastagens
        </Link>
