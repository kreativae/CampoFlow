...
export default function PasturesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<Pasture[]>(`/farms/${farmId}/pastures`, { token: accessToken });
      setPastures(data);
    } catch (err) {
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch<Pasture>(`/farms/${farmId}/pastures`, {
        method: 'POST',
        token: accessToken,
        body: {
          name,
      ...
      {fetching ? (
        ...
        <ul className="space-y-2">
          {pastures.map((pasture) => (
            <li key={pasture.id}>
              <Link
                href={`/farms/${farmId}/pastures/${pasture.id}`}
