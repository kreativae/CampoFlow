...
export default function QuotationsPage() {
  ...
  const loadHistory = useCallback(async (commodity: Commodity) => {
    try {
      const data = await apiFetch<Quotation[]>(`/quotations?commodity=${commodity}`, {
        token: accessToken,
      });
      setHistory(data);
    } catch (err) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const latestData = await apiFetch<LatestQuotation[]>('/quotations/latest', {
        token: accessToken,
      });
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiFetch<Quotation>('/quotations', {
        method: 'POST',
        token: accessToken,
